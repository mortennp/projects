...
double getSonar(){   // get the measurement results of ultrasonic module,with unit: cm
    digitalWrite(trigPin,HIGH); //trigPin send 10us high level 
    delayMicroseconds(10);
    digitalWrite(trigPin,LOW);
    long pingTime = pulseIn(echoPin,HIGH,timeOut);   //read plus time of echoPin
    double distance = pingTime * 340.0 / 2.0 / 10000.0; // the sound speed is 340m/s,and calculate distance
    return distance;
}
